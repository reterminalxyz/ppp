...
export const InteractiveHero: React.FC = () => {
  ...
  // State to track manual rotation from touch/drag
  const [manualRotation, setManualRotation] = useState({ x: 0, y: 0 });
  const isDragging = useRef(false);
  const lastPos = useRef({ x: 0, y: 0 });

  // Handle Mouse Move (Desktop Hover)
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    objectRef.current.style.transform = `rotateX(${rotateX + manualRotation.x}deg) rotateY(${rotateY + manualRotation.y}deg)`;
  };

  const handleMouseLeave = () => {
    if (isDragging.current || !objectRef.current) return;
    objectRef.current.style.transform = `rotateX(${manualRotation.x}deg) rotateY(${manualRotation.y}deg)`;
  };

  // Handle Touch / Drag (Mobile & Desktop Dragging)
  ...
  const handlePointerMove = (e: TouchEvent | MouseEvent) => {
    ...
    // Prevent scrolling while rotating the cactus on mobile
    if ('touches' in e) {
      e.preventDefault();
    }
    ...
    // Adjust sensitivity
    const newRotX = manualRotation.x - deltaY * 0.5;
    const newRotY = manualRotation.y + deltaX * 0.5;

    setManualRotation({ x: newRotX, y: newRotY });
  ...
  // Attach non-passive touchmove listener to prevent scrolling while interacting
  useEffect(() => {
  ...
  return (
    <div 
      className="relative w-full h-[70vh] md:h-[80vh] flex items-center justify-center overflow-hidden border-b-4 border-mex-pink bg-mex-dark touch-none"
    ...
    >
      ...
      >
        {/* 
          ...
          Increased sizes for mobile: w-[280px] h-[420px] on small screens.
        */}
        <div className="relative w-[280px] h-[420px] sm:w-[320px] sm:h-[480px] md:w-[400px] md:h-[600px] animate-spin-3d" style={{ transformStyle: 'preserve-3d' }}>
          {planes.map((deg) => (
      ...
      {/* Mobile hint */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-mex-orange font-mono text-xs opacity-50 md:hidden pointer-events-none">
        [ SWIPE TO ROTATE ]
      </div>
    </div>
  );
